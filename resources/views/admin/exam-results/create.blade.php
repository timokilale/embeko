...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Add New Exam Result</h1>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('admin.exam-results.store') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="row mb-3">
                    <div class="col-md-4">
                        <label for="exam_name" class="form-label">Exam Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('exam_name') is-invalid @enderror" id="exam_name" name="exam_name" value="{{ old('exam_name') }}" list="exam-suggestions" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control @error('class') is-invalid @enderror" id="class" name="class" value="{{ old('class') }}" list="class-suggestions" required>
                ...
                <div class="mb-3">
                    ...
                    @error('results_file')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <div class="form-text">Upload a PDF, Excel, or Word document containing the results. Maximum file size: 10MB.</div>
                </div>

                <div class="mb-3">
                    <label for="results_data" class="form-label">Results Data</label>
                    <textarea class="form-control @error('results_data') is-invalid @enderror" id="results_data" name="results_data" rows="10">{{ old('results_data') }}</textarea>
                    @error('results_data')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <div class="form-text">Enter the results data directly if not uploading a file. You can use HTML formatting.</div>
                </div>
