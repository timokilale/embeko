...
@section('title', 'Edit Leader - ' . $leader->name)

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3">Edit Leader</h1>
            ...
            <form action="{{ route('admin.leaders.update', $leader->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="5">{{ old('description', $leader->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3 form-check">
                            <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1" {{ old('is_active', $leader->is_active) ? 'checked' : '' }}>
                            ...
                            <div class="mb-3">
                                <div class="d-flex justify-content-center mb-3">
                                    <div class="image-preview bg-light d-flex align-items-center justify-content-center" style="width: 200px; height: 200px; border-radius: 50%; overflow: hidden;">
                                        @if($leader->image)
                                            <img id="preview-image" src="{{ asset('storage/' . $leader->image) }}" alt="{{ $leader->name }}" style="width: 100%; height: 100%; object-fit: cover;">
                                            <i class="fas fa-user fa-3x text-secondary" id="placeholder-icon" style="display: none;"></i>
                                        @else
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save me-2"></i>Update Leader
                    </button>
                </div>
            </form>
        </div>
